...
hook的基本原理在于React提供并维护了一个全局的公共对象dispatcher，在不同的声明周期其为hook提供不同的处理方法，函数组件代码中看上去调用了同一个 Hooks API，但实际上会在不同时期取到不同的方法，也就实现了以前 Class 写法中的生命周期的效果。
...
- 客户端获取数据：客户端通过fetch访问api获取数据。页面包含经常更新的数据，并且您不需要预先呈现数据，则可以在客户端获取数据，例如用户特定的数据。